...
namespace xpp {
  ...
  public:
      ...
      throw(XmlPullParserException) 
    {
      ...
      if(qName[i] == _T('\0'))   //current default namespace 
     ...
     */
    void setSupportNamespaces(bool enable) throw(XmlPullParserException) {
    ...
    int skipSubTree() throw(XmlPullParserException) {
      int level = 1;
      StartTag stag;
    ...
    int next() throw(XmlPullParserException) {
     ...
     try {
       while(true) {
         ...
         case XmlTokenizer::ETAG_NAME: {
           ...
           tokenizer.buf[tokenizer.posEnd] = _T('\0');
           --elStackDepth;
           if(elStackDepth == 0)
             seenRootElement = true;
           if(elStackDepth < 0) 
    ...
    const bool whitespaceContent() throw(XmlPullParserException) {
    ...
    const SXT_CHAR* readContent() throw(XmlPullParserException) {
      if(eventType != CONTENT) {
        throw XmlPullParserException("no content available to read");
      }
      ...
      if(elContent == NULL) {
        if(tokenizer.parsedContent) {
          ...
          tokenizer.pc [ tokenizer.pcEnd ] = _T('\0');
        } else {
    ...
    void readEndTag(EndTag& etag) const throw(XmlPullParserException)  {
    ...
    void readStartTag(StartTag& stag) throw(XmlPullParserException) {
